...
CLOUDSQL_INSTANCE=$6
CLOUDSQL_DATABASE=$7
TABLES=$8  # Comma-separated list of tables

# Validate parameters
if [[ -z "$MYSQL_USER" || -z "$MYSQL_PASSWORD" || -z "$MYSQL_HOST" || -z "$MYSQL_DATABASE" || -z "$GCS_BUCKET" || -z "$CLOUDSQL_INSTANCE" || -z "$CLOUDSQL_DATABASE" ]]; then
  echo "Usage: $0 <MYSQL_USER> <MYSQL_PASSWORD> <MYSQL_HOST> <MYSQL_DATABASE> <GCS_BUCKET> <CLOUDSQL_INSTANCE> <CLOUDSQL_DATABASE> [<TABLES>]"
  exit 1
fi
...
# Step 1: Create MySQL Dump for Specific Tables and Upload to GCS
echo "Creating MySQL dump for tables and uploading to GCS..."
if [[ -z "$TABLES" ]]; then
  # Dump the entire database if no specific tables are provided
  mysqldump -u "$MYSQL_USER" -p"$MYSQL_PASSWORD" -h "$MYSQL_HOST" --databases "$MYSQL_DATABASE" | gsutil cp - "gs://$GCS_BUCKET/$SQL_FILE"
else
  # Dump specific tables if provided (replace commas with spaces)
  TABLES_SPACED=$(echo $TABLES | tr ',' ' ')
  mysqldump -u "$MYSQL_USER" -p"$MYSQL_PASSWORD" -h "$MYSQL_HOST" "$MYSQL_DATABASE" $TABLES_SPACED | gsutil cp - "gs://$GCS_BUCKET/$SQL_FILE"
fi
